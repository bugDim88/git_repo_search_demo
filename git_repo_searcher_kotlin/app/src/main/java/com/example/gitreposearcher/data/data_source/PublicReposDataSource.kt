...
class PublicReposDataSource(private val service: GitHubService) :
    ReposDataSource() {
    ...
    override fun loadInitial(
        ...
        try {
            val response = service.getPublicRepos().execute()
    ...
    override fun loadAfter(params: LoadParams<String>, callback: LoadCallback<String, RepoModel>) {
        ...
        try {
            val response = service.getPublicRepos(params.key).execute()
            if (!response.isSuccessful) throw Throwable(response.message())
            val repos = response.body()
                ?.map {
                    RepoModel(
